...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(window_width, window_height, "Comp371 - Project - Team 4", NULL, NULL);
    if (window == NULL)
    {
    ...
    BlackHoleRoomScene* r = new BlackHoleRoomScene(window_width, window_height);
    BlackHoleRoomScene* r2 = new BlackHoleRoomScene(window_width, window_height);
...
void mouse_movement_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if(bLockCursor)
    {
        ...
        yaw += -x_diff;
        pitch += y_diff;
        //std::cout << x_diff << " x_diff\t\t" << y_diff << " ydiff" << std::endl;
